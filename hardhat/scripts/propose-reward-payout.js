...
function storeProposalId(proposalId) {
  ...
  fs.writeFileSync(proposalsFile, JSON.stringify(proposals), "utf8");
    ...
    const hypetribeTKN = await ethers.getContract("HypetribeTKN");
    const encodedFunctionCall = hypetribeTKN.interface.encodeFunctionData(functionToCall, args);
    console.log(`Proposing ${functionToCall} on ${hypetribeTKN.address} with ${args}`);
    console.log(`Proposal Description:\n${proposalDescription}`);
    const proposeTx = await governor.propose(
        [hypetribeTKN.address],
        [0],
        [encodedFunctionCall],
        proposalDescription
      )
